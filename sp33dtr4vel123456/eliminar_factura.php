	...
	<body>
		...
		<!-- Main -->
			<div id="main" class="wrapper style1">
				<div class="container">
					<header class="major">
						<h2>Seleccione una factura a eliminar</h2>	
						<form method="post" action="eliminar_factura2.php">			
								<select class="negro" size="1" name="numero">
      							<?php
								require_once("../connect.php");
                                require_once("../validadores.php");
                                $result = mysql_query("SELECT numero, orden_de_compra, valor FROM facturas");            
                                while ($row = mysql_fetch_row($result))
                                {
                                    echo "<option value='$row[0]'>Número: $row[0] - Orden de compra: $row[1] - Valor: $row[2]</option>";   
                                }
								?>
     							</select><br>
     							<div class="12u$">
										<ul class="actions">
											<li><input type="submit" value="Eliminar factura" class="special" onclick="return confirm('¿Seguro desea borrar esta factura?')"/></li>
										</ul>
								</div>	
						</form>
					</div>
