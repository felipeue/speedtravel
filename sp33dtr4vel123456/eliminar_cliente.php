			...
			<div id="main" class="wrapper style1">
				<div class="container">
					<header class="major">
						...
						<h2>Seleccione un cliente para eliminar</h2>	
						<form method="post" action="eliminar_cliente2.php">			
								<select class="negro" size="1" name="rut">
      							<?php
								require_once("../connect.php");
                                require_once("../validadores.php");
                                $result = mysql_query("SELECT nombre, apellido, rut, empresa FROM user ");            
                                while ($row = mysql_fetch_row($result))
                                {
                                    ...
                                    echo "<option value='$row[2]'>Nombre: $nombre $apellido - Rut: $row[2] - Empresa: $row[3]</option>";   
                                }
								?>
     							</select><br>
     							<div class="12u$">
										<ul class="actions">
											<li><input type="submit" value="Borrar" class="special" onclick="return confirm('¿Seguro desea borrar este cliente')" /></li>
										</ul>
								</div>	
						</form>
					</div>
