			...
			<div id="main" class="wrapper style1">
				<div class="container">
					<header class="major">
						...
						<p>Seleccione un pasaje pendiente a usar [Nombre | Rut | Codigo Reserva | Nº Boleto ]</p>
						
						<form method="post" action="pasaje_pendiente2.php">			
								<select class="negro" size="1" name="pasaje">
      							<?php
								require_once("../connect.php");
                                require_once("../validadores.php");
                                $result = mysql_query("SELECT pasajero.nombre, pasajero.rut, pasaje_modificado.codigo_reserva, pasaje_modificado.numero_boleto, pasaje_modificado.id, pasajero.apellido  FROM pasajero, pasaje_modificado WHERE pasajero.rut = pasaje_modificado.rut_pasajero");            
								...
								?>
     							<div class="12u$">
										<ul class="actions">
											<li><input type="submit" onclick="return confirm('Usar este pasaje?');" value="Usar Pasaje" class="special" /></li>
										</ul>
								</div>	
						</form>
					</div>
