			...
			<div id="main" class="wrapper style1">
				<div class="container">
					<header class="major">
						...
						<p>Seleccione una orden de compra para eliminar</p>	
						<form method="post" action="eliminar_odc2.php">			
								<select class="negro" size="1" name="numero">
      							<?php
								require_once("../connect.php");
                                require_once("../validadores.php");
                                $result = mysql_query("SELECT empresa, numero, valor_inicial, dia,mes,ano FROM orden_de_compra");            
                                while ($row = mysql_fetch_row($result))
                                {
                                	$empresa = ucwords($row[0]);
                                	
                                    echo "<option value='$row[1]'>Empresa: $empresa - Número: $row[1] - Valor: $$row[2] - Fecha: $row[3]/$row[4]/$row[5]</option>";   
                                }
								?>
     							</select><br>
     							<div class="12u$">
										<ul class="actions">
											<li><input type="submit" value="Borrar" class="special" onclick="return confirm('¿Seguro desea borrar esta orden de compra?')" /></li>
										</ul>
								</div>	
						</form>
					</div>
